...
function preChecks(dotenv, env, DEV_SERVER_AUTO_OPEN, SEARCH_ENGINES) {
    ...
    console.log(
        ` | (___   __ _ _   _  __ _  __| | |  __ _   _  ___  ___ ___ _ __ `,
    );
    console.log(
        `  \\___ \\ / _\` | | | |/ _\` |/ _\` | | |_ | | | |/ _ \/ __/ __| '__|`,
    );
    console.log(
        `  ____) | (_| | |_| | (_| | (_| | |__| | |_| |  __/\\__ \\__ \\ |   `,
    );
    console.log(
        ` |_____/ \\__, |\\__,_|\\__,_|\\__,_|\\_____|\\__,_|\\___||___/___/_|   `,
    );
    console.log(
        `            | |                                                  `,
    );
    ...
    console.log(
        `                            https://github.com/sh4rkman/SquadGuessr`,
    );
    console.log(
        "*******************************************************************\n",
    );
    ...
    if (dotenv.error) {
        ...
        console.error(
            "    -> see https://github.com/sh4rkman/SquadCalc/wiki/Installation-&-Configuration#optional-configuration\n\n",
        );
        process.exit(1);
    }
    ...
    console.log(
        `      -> Index on search engines : ${SEARCH_ENGINES ? "✅" : "❌"}`,
    );
    if (!env.WEBPACK_BUILD)
        console.log(
            `      -> URL will be http://localhost:${process.env.DEV_SERVER_PORT || 3000} ✅`,
        );
    ...
    if (!env.WEBPACK_BUILD)
        console.log(
            `      -> Should dev server open in a new Tab ? ${DEV_SERVER_AUTO_OPEN ? "✅" : "❌"}`,
        );
    console.log(
        `    -> ${env.WEBPACK_BUILD ? "Building /dist/ folder...\n" : "Launching dev server...\n"}`,
    );
}
